...
/**
 * Delete all domain keywords for specified domains
 * @param clientCode - The client code to filter by
 * @param locationCode - Optional location code to filter by
 * @param domains - Array of domain names to delete keywords for
 * @returns The count of deleted records
 */
export async function deleteDomainKeywordsByDomains(
  clientCode: string,
  ...
  const filteredRecords = allRecords.filter(r => {
    // If record doesn't match the client, keep it
    if (r.clientCode !== clientCode) return true;
    // If locationCode provided and doesn't match, keep it
    if (locationCode && r.locationCode !== locationCode) return true;
    // If domain is in the delete list, remove it
    if (domains.includes(r.domain)) return false;
    // Otherwise keep it
    return true;
  });

  const deletedCount = originalCount - filteredRecords.length;

  if (deletedCount > 0) {
    await writeDomainKeywords(filteredRecords);
  }
